...
fn cache_miss_then_hit() {
    ...
    assert!(cache.get("test query", &["slack"], 20).is_none());
    let response = make_response(3);
    cache.put("test query", &["slack"], 20, response);
    let cached = cache.get("test query", &["slack"], 20);
...
fn cache_ttl_expiry() {
    ...
    assert!(cache.get("query", &["slack"], 20).is_some());
    std::thread::sleep(Duration::from_millis(60));
    assert!(cache.get("query", &["slack"], 20).is_none());
}
...
fn cache_eviction_at_max() {
    ...
    for i in 0..4 {
        cache.put(&format!("query{}", i), &["slack"], 20, make_response(1));
    }
    // Should have exactly 3 entries
    let present: usize = (0..=3)
        .filter(|i| cache.get(&format!("query{}", i), &["slack"], 20).is_some())
        .count();
    assert_eq!(present, 3);
}
...
fn different_sources_different_keys() {
    let mut cache = ResponseCache::new(100, Duration::from_secs(300));
    cache.put("query", &["slack"], 20, make_response(1));
    cache.put("query", &["jira"], 20, make_response(2));
    assert_eq!(cache.get("query", &["slack"], 20).unwrap().results.len(), 1);
    assert_eq!(cache.get("query", &["jira"], 20).unwrap().results.len(), 2);
}
...
#[test]
fn different_max_results_different_keys() {
    let mut cache = ResponseCache::new(100, Duration::from_secs(300));
    cache.put("query", &["slack"], 10, make_response(1));
    cache.put("query", &["slack"], 20, make_response(2));
    assert_eq!(cache.get("query", &["slack"], 10).unwrap().results.len(), 1);
    assert_eq!(cache.get("query", &["slack"], 20).unwrap().results.len(), 2);
}
