...
- **Rust char-index ≠ byte-index**: never use `Vec<char>` indices to slice `&str`/`&input[..]` — panics `"byte index N is not a char boundary"` on the first multi-byte codepoint (emoji, em-dash). For ASCII-delimiter scans (HTML/XML: `< > ' " = /` + whitespace), iterate `input.as_bytes()` instead — UTF-8 continuation bytes (0x80-0xBF) never collide with ASCII delimiters, so byte indices land on codepoint boundaries automatically. v0.3.4 hung 2 days on a Confluence page containing ✅.
- **Panic across an async boundary can wedge the runtime** — a panic in a future polled by `tokio::spawn` (or rmcp's per-tool dispatch) kills the worker; outer code awaiting it may stall indefinitely with no error surfaced. Wrap CPU-bound or panic-prone work in `tokio::task::spawn_blocking` and inspect the `JoinHandle`: `match handle.await { Ok(v) => …, Ok(Err(join_err)) | Err(join_err) => fallback }`. Pattern used in `confluence.rs:get_detail_page` for `to_markdown`.
...
- `cargo test` runs ~212 tests across 16 test files. All must pass before committing.
...
- **Reproduce MCP behavior at the shell** — pipe a 3-message JSON-RPC sequence (`initialize`, `notifications/initialized`, `tools/call`) into `./target/release/unified-search-mcp --config config.yaml`, append a `sleep 70` after the calls so stdin stays open long enough for the response. Watch stderr (`RUST_LOG=unified_search_mcp=info`) for per-call tracing. Faster than restarting a Claude session — and the only way to debug the killing session's MCP after a respawn.
- **Force MCPs to pick up a new binary**: `pkill -f 'unified-search-mcp/target/release'` (or `ps aux | grep 'unified-search-mcp/target/release' | grep -v grep | awk '{print $2}' | xargs kill`). Each Claude session respawns its MCP on the next tool call. The session that runs the kill loses unified-search until restarted — run from a session you don't mind ending.
- **Inject MCP env vars into the current shell** for direct binary testing: `eval "$(python3 -c "import json,os; e=json.load(open(os.path.expanduser('~/.claude.json')))['mcpServers']['unified-search']['env']; [print(f'export {k}={v!r}') for k,v in e.items()]")"`. Avoids hardcoding tokens in scripts; pulls from the same source Claude Code uses.
